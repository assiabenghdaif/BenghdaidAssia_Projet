...
//        }
    ...
    public History HistoryByAll(String email,String activity,String dateDe,String dateFi){
        ...
        // Define a selection and arguments for the WHERE clause
        String selection = "email_user = ? and activity=? and dateDe=? and dateFi=?";
        String[] selectionArgs = { String.valueOf(email), String.valueOf(activity),String.valueOf(dateDe),String.valueOf(dateFi)};
        ...
        if (cursor.moveToFirst()) {
            // Extract the image data from the cursor
            history =new History(cursor.getDouble(0),cursor.getDouble(1),cursor.getDouble(2),cursor.getDouble(3));
        }
...
//        }
    ...
    public boolean insertHistory (String email, History history) {
//        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(DatabaseHelper.EMAIL_HIS, email);
        contentValues.put(DatabaseHelper.dateDe, history.getDateDe());
        contentValues.put(DatabaseHelper.dateFi, history.getDateFi());
        contentValues.put(DatabaseHelper.ACTIVITY, history.getActivity());
        contentValues.put(DatabaseHelper.LatitudeFinish, history.getLatitudeFinish());
        contentValues.put(DatabaseHelper.LatitudeStart, history.getLatitudeStart());
        contentValues.put(DatabaseHelper.LongitudeStart, history.getLongitudeStart());
        contentValues.put(DatabaseHelper.LongitudeFinish, history.getLongitudeFinish());
